...
const api = {
	...
	showItemInFolder: (targetPath) =>
		ipcRenderer.invoke("desktop:show-item-in-folder", targetPath),
	...
	writeTempFile: (name, data) =>
		ipcRenderer.invoke("desktop:write-temp-file", { name, data }),
	removePath: (filePath) =>
		ipcRenderer.invoke("desktop:remove-path", { filePath }),
	readFile: (filePath) =>
		ipcRenderer.invoke("desktop:read-file", { filePath }),

	ffmpegRun: (args) => ipcRenderer.invoke("ffmpeg:run", { args }),
	ffmpegStartPipe: (args, id) =>
		ipcRenderer.invoke("ffmpeg:start-pipe", { args, id }),
