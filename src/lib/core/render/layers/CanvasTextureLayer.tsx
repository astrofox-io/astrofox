...
export function CanvasTextureLayer({
  ...
  React.useLayoutEffect(() => {
    ...
    const nextWidth = Math.max(1, Math.round(firstPass.width || canvas.width || 1));
    const nextHeight = Math.max(1, Math.round(firstPass.height || canvas.height || 1));
    ...
    const nextOriginX =
      firstPass.originX !== undefined ? firstPass.originX : Math.round(nextWidth / 2);
    const nextOriginY =
      firstPass.originY !== undefined ? firstPass.originY : Math.round(nextHeight / 2);
