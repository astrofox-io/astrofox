...
export default function AuthScreen() {
	...
	return (
		<div className={"min-h-screen flex items-center justify-center bg-[radial-gradient(circle_at_top,_#1f2a38_0%,_#101014_70%)]"}>
			<form className={"w-96 flex flex-col gap-2.5 p-6 bg-[rgba(14,_20,_28,_0.92)] border border-[#2f3f55]"} onSubmit={handleSubmit}>
				<div className={"text-2xl font-bold tracking-wide"}>Astrofox</div>
				<div className={"text-sm opacity-[0.8] mb-2"}>
					{mode === "sign-up" ? "Create account" : "Sign in"}
				</div>

				{mode === "sign-up" && (
					<input
						className={"bg-[#0d1219] text-[#fff] border border-[#3c4b60] py-2 px-2.5 text-sm"}
						type="text"
						placeholder="Name"
						value={name}
