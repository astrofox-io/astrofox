...
export default class CanvasBars extends Entity {
  ...
  render(data: Float32Array | number[]) {
    ...
    // Reset canvas
    resetCanvas(canvas, width as number, (height as number) + (shadowHeight as number));
    ...
    for (let i = 0, x = 0, last = null; i < bars && x < fullWidth; i += step, x += barSize) {
      ...
      if (index !== last) {
        const val = clamp(data[index] * (height as number), minHeight as number, height as number);
        last = index;
