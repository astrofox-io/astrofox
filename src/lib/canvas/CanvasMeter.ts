...
export default class CanvasMeter extends Entity {
  ...
  render(value: number) {
    ...
    const { height, width, color, origin } = this.properties as Record<string, unknown>;
    ...
    switch (origin) {
      case 'left':
        ...
        break;
      case 'bottom':
        context.fillRect(0, height as number, width as number, ~~(-value * (height as number)));
        break;
      case 'right':
        context.fillRect(width as number, 0, ~~(-value * (width as number)), height as number);
        break;
      case 'top':
        context.fillRect(0, 0, width as number, ~~(-value * (height as number)));
        break;
    }
  }
}
