...
export default function VideoSettings({ onClose }) {
	const isWeb = env.IS_WEB;
	const [audioFile, duration] = useAudio(
		(state) => [state.file, state.duration],
		shallow,
	);
	const [state, setState] = useState(initialState);
	const { videoFile, codec, fps, quality, timeStart, timeEnd } = state;
	const canStart = !isWeb && videoFile && audioFile && timeEnd - timeStart > 0;
	...
	async function handleOpenVideoFile() {
		...
		if (!canceled) {
			const name =
				filePath || fileHandle?.name || `video-${Date.now()}.${extension}`;
			setState((state) => ({
				...state,
				videoFile: replaceExt(name, `.${extension}`),
			}));
		}
	}
	...
	return (
		<Layout width={700}>
			<Settings columns={["40%", "60%"]} onChange={handleChange}>
				{isWeb && (
					<Setting label="Notice">
						Video export requires a desktop build and is not available in the
						web version.
					</Setting>
				)}
				<Setting
