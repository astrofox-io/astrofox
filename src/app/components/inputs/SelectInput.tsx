...
export default function SelectInput({
	...
	return (
		...
		>
			<SelectTrigger
				className={cn(
					"h-auto cursor-default text-sm text-neutral-300 bg-neutral-900 border-neutral-600 rounded-md py-1 px-2 shadow-none",
					className,
				)}
				style={{ width }}
			>
			...
			<SelectContent
				className={cn(
					"bg-neutral-900 border-neutral-700 rounded-md shadow-lg p-1",
				)}
			>
				{parsedItems.map((item: SelectItemData | null, index: number) => {
					...
					return (
						<SelectItem
							key={index}
							value={itemValue}
							className="text-neutral-300 text-sm py-1 px-2 min-w-36 rounded-md focus:text-neutral-100 focus:bg-primary"
