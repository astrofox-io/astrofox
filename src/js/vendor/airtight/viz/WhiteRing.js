...
var WhiteRing = function() {
	...
	function init(){
		...
		//INIT CONTROLS
		var folder = ControlsHandler.getVizFolder().addFolder('White Ring');
	...
	function onToggleViz(){
		...
		if (vizParams.on){
			...
			//empty square
			geometry = new THREE.RingGeometry( radius*thickness,radius, 4,1, 0, Math.PI*2) ;
			...
			//empty tri
			geometry = new THREE.RingGeometry( radius*thickness,radius, 3,1, 0, Math.PI*2) ;
			mesh = new THREE.Mesh( geometry, material );
			groupHolder.add( mesh );
			shapes.push(mesh);

			//empty circ
			geometry = new THREE.RingGeometry( radius*thickness,radius, 24,1, 0, Math.PI*2) ;
			mesh = new THREE.Mesh( geometry, material );
			groupHolder.add( mesh );
			shapes.push(mesh);
	...
	function update() {
		...
		var gotoScale = AudioHandler.getSmoothedVolume()*.6 + .2;
